...
function _applyUiTheme(themeId) {
  ...
  if (btn) {
    ...
    btn.title = `Theme: ${t.id}. Click to cycle (Meridian → CRT → Neon → Raw). Saved locally.`;
  }
}
...
function _initUiTheme() {
  let saved = "meridian";
  try {
    saved = localStorage.getItem(UI_THEME_KEY) || "meridian";
  } catch (_) { /* ignore */ }
  if (!UI_THEMES.some(x => x.id === saved)) saved = "meridian";
  _applyUiTheme(saved);
  document.getElementById("theme-cycle-btn")?.addEventListener("click", () => {
    let cur = "meridian";
    try {
      cur = localStorage.getItem(UI_THEME_KEY) || "meridian";
    } catch (_) { /* ignore */ }
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", _initUiTheme);
} else {
  _initUiTheme();
}
