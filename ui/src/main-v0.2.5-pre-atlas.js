...
const UI_THEMES = [
  // Default: Control Room (beyond terminal)
  { id: "controlroom", label: "⌬ ROOM", cls: "theme-controlroom" },
  // Default: Starship‑OS (full shell redesign)
  { id: "starshipos", label: "✦ OS",  cls: "theme-starshipos" },
  // v4 Spatial HUD layout (previous default)
  { id: "hud",        label: "◈ HUD", cls: "theme-hud" },
  // Alternate: macOS-style layout
  { id: "orchard",    label: "⌘ ORCH", cls: "theme-orchard" },
  // Previous experiments / backups
  { id: "nuke",       label: "✹ NUKE", cls: "theme-nuke" },
  { id: "crt",        label: "█ CRT",  cls: "theme-crt" },
  { id: "neon",       label: "✦ NEON", cls: "theme-neon" },
  { id: "brutal",     label: "▮ RAW",  cls: "theme-brutal" },
];

function _applyUiTheme(themeId) {
  const t = UI_THEMES.find(x => x.id === themeId) || UI_THEMES[0];
  for (const x of UI_THEMES) {
    if (x.cls) document.body.classList.remove(x.cls);
  }
  ...
  if (btn) {
    ...
    btn.title = `Theme: ${t.id}. Click to cycle (ROOM → OS → HUD → Orchard → Nuke → CRT → Neon → Raw). Saved locally.`;
  }
}

function _initUiTheme() {
  let saved = "controlroom";
  try {
    saved = localStorage.getItem(UI_THEME_KEY) || "controlroom";
  } catch (_) { /* ignore */ }
  if (!UI_THEMES.some(x => x.id === saved)) saved = "controlroom";
  _applyUiTheme(saved);
  document.getElementById("theme-cycle-btn")?.addEventListener("click", () => {
    let cur = "controlroom";
    try {
      cur = localStorage.getItem(UI_THEME_KEY) || "controlroom";
    } catch (_) { /* ignore */ }
...
// ── Resizable columns ────────────────────────────────────────────────────────
const COL_STORAGE_KEY = "att_col_widths_v2";
const COL_MIN = [100, 220, 250, 200]; // min px for scanner, left, centre, right

function _defaultColWidths() {
  return [150, null, null, 280]; // null = flex; centre guaranteed minmax(280px, 1.2fr)
}
...
function _applyColWidths(w) {
  ...
  if (!ws) return;
  const c0 = w[0] != null ? `${w[0]}px` : "150px";
  const c1 = w[1] != null ? `${w[1]}px` : "1fr";
  const c2 = w[2] != null ? `${w[2]}px` : "minmax(280px, 1.2fr)";
  const c3 = w[3] != null ? `${w[3]}px` : "280px";
  ws.style.gridTemplateColumns = `${c0} 5px ${c1} 5px ${c2} 5px ${c3}`;
}
...
function _wireControlRoomNav() {
  const nav = document.querySelector(".cr-nav");
  const canvas = document.querySelector(".cr-canvas");
  if (!nav || !canvas) return;

  const btns = Array.from(nav.querySelectorAll(".cr-nav-btn"));
  if (!btns.length) return;

  const targets = [
    () => canvas.querySelector(".cr-canvas-top"),
    () => document.getElementById("col-left"),
    () => document.getElementById("news-tape") || document.getElementById("reasoning-panel"),
    () => document.getElementById("order-ticket"),
    () => document.getElementById("theme-cycle-btn"),
  ];

  const setActive = (i) => {
    btns.forEach((b, idx) => b.classList.toggle("cr-nav-active", idx === i));
  };
...
function _openNewsModal(item) {
  ...
  const impactsContainer = document.getElementById("news-modal-impacts")
    || (() => {
      const d = document.createElement("div");
      d.id = "news-modal-impacts";
      d.className = "news-modal-impacts";
      sumEl.after(d);
      return d;
    })();

  // Fetch cross-stock data for this article's tickers
  const articleTickers = (item.tickers || []).filter(t => t && t.length <= 6);
  if (articleTickers.length > 0) {
    impactsContainer.innerHTML = '<div class="news-modal-impacts-title">CROSS-STOCK IMPACTS</div><div style="color:#3a3a52;font-size:9px">Loading…</div>';
    impactsContainer.style.display = "";
...
document.addEventListener("DOMContentLoaded", () => {
  const sel = document.getElementById("news-sort");
  if (!sel) return;
...
async function _fetchArticleImpacts(tickers) {
  ...
  try {
    for (const t of tickers.slice(0, 3)) {
      const r = await fetch(`${BACKEND}/news/impacts/${t}`, { signal: AbortSignal.timeout(2000) });
      if (r.ok) {
        const data = await r.json();
...
function renderNews(items) {
  ...
  // Add new items into buffer (deduped), then re-render based on selected sort.
  // Keep buffer bounded.
...
function appendReasoningEntry(entry) {
  ...
  // Derive a clean role key for CSS color coding
...
function updateTierBar(tiers) {
  ...
  // Sentiment
  const sent = document.getElementById("tb-sentiment");
  ...
  // Movement
  const mov = document.getElementById("tb-movement");
  ...
  // Last run time
  const lr = document.getElementById("tb-last-run");
...
document.getElementById("tb-run-t3")?.addEventListener("click", async () => {
  const btn = document.getElementById("tb-run-t3");
  const st  = document.getElementById("tb-t3-status");
...
async function pollRecommendations() {
  try {
    const r = await fetch(`${BACKEND}/recommendations`, { signal: AbortSignal.timeout(3000) });
    if (!r.ok) return;
...
function renderRecommendations(recs) {
  ...
  const recent  = recs.filter(r => r.status !== "pending").slice(-5);

  if (count) count.textContent = pending.length > 0 ? `${pending.length} pending` : "";

  if (pending.length === 0 && recent.length === 0) {
    panel.innerHTML = '<div class="rec-empty">No pending recommendations</div>';
    return;
  }

  const cards = [...pending, ...recent].map(rec => {
    ...
    const actionsHtml = isPending
      ? `<div class="rec-card-actions">
           <button class="rec-btn rec-btn-approve" data-rec-id="${esc(rec.id)}" data-action="approve">APPROVE</button>
           <button class="rec-btn rec-btn-dismiss" data-rec-id="${esc(rec.id)}" data-action="dismiss">DISMISS</button>
         </div>`
      : `<span class="rec-status-${rec.status === "approved" ? "approved" : "dismissed"}">${rec.status.toUpperCase()}</span>`;

    const riskLine = rec.proposal
      ? `MAX RISK $${Number(rec.proposal.max_risk || 0).toFixed(0)} · TARGET $${Number(rec.proposal.target_return || 0).toFixed(0)}`
      : "";

    return `
      <div class="rec-card" data-rec-status="${rec.status}">
        <div class="rec-card-header">
          <span class="rec-card-ticker">${esc(rec.ticker)}</span>
          <span class="rec-card-strategy">${esc(rec.strategy_name)}</span>
          <span class="rec-card-confidence ${confClass}">${(rec.confidence * 100).toFixed(0)}%</span>
        </div>
        <div class="rec-card-convictions">
          <span class="rec-conv-bull">BULL ${rec.bull_conviction}/10</span>
          <span class="rec-conv-bear">BEAR ${rec.bear_conviction}/10</span>
        </div>
        <div class="rec-card-reasoning">${esc(rec.desk_head_reasoning || "").slice(0, 200)}</div>
        <div class="rec-card-risk">${riskLine}</div>
        ${actionsHtml}
      </div>`;
  });
  ...
  panel.querySelectorAll(".rec-btn").forEach(btn => {
    btn.addEventListener("click", async (e) => {
      const id     = e.target.dataset.recId;
      const action = e.target.dataset.action;
      if (!id || !action) return;
      e.target.disabled = true;
      e.target.textContent = "…";
      try {
        await fetch(`${BACKEND}/recommendations/${id}/${action}`, { method: "POST" });
        pollRecommendations();
      } catch { /* ignore */ }
    });
  });
...
async function pollNewsImpacts() {
  try {
    const r = await fetch(`${BACKEND}/news/impacts`, { signal: AbortSignal.timeout(4000) });
    if (!r.ok) return;
    const data = await r.json();
...
function renderImpactStrip(impacts) {
  const strip = document.getElementById("news-impact-strip");
  const countEl = document.getElementById("news-processed-count");
  if (!strip) return;
  ...
  strip.innerHTML = entries.map(e => {
    ...
    const sign = score > 0 ? "+" : "";
    return `<span class="impact-chip" title="${esc(e.ticker)}: ${e.article_count} articles — ${(e.relationships || []).slice(0, 2).join(', ')}">
...
async function pollNewsFeed() {
  try {
    const r = await fetch(`${BACKEND}/news?limit=80`, { signal: AbortSignal.timeout(4000) });
    if (!r.ok) return;
...
setInterval(pollNewsFeed,       30000);   // news headlines (new articles arrive every ~5min)
